...
// Verzion 用于找到一个更长的区块链。当一个节点接收到 version 消息，它会检查本节点的区块链是否比 BestHeight 的值更大。如果不是，节点就会请求并下载缺失的块。
type Version struct {
	// 版本 实际并不会存储什么重要信息
	Version int
	// 存储区块链中节点的高度
...
func SendTxServer(tnx *block.Transaction) {
...
func handleBlock(request []byte, bc *block.Blockchain) {
	...
	b := block.DeserializeBlock(blockData)
...
func handleConnection(conn net.Conn, bc *block.Blockchain) {
	...
	case "addr":
		handleAddr(request)
	case "block":
		// 当接收到一个新块时，我们把它放到区块链里面。如果还有更多的区块需要下载，
		// 我们继续从上一个下载的块的那个节点继续请求。当最后把所有块都下载完后，对 UTXO 集进行重新索引。
...
func StartServer(nodeID, minerAddress string) {
	...
	// 比较当前节点是否中心节点 如果不是中心节点需要请求中心节点查询当前区块链是否过时
	if nodeAddress != knownNodes[0] {
